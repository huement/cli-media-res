...
def upload_directory(local_dir, target_extension=None, remote_prefix=None, webp_quality=80):
    ...
    print(f"WebP Conversion: Enabled (JPG/PNG -> WEBP)")
    print(f"==========================================\n")
    ...
    for root, _, files in os.walk(local_dir):
        for file in files:
            ...
            # --- WebP Conversion Logic for PNG and JPG/JPEG ---
            if file_ext in ['.png', '.jpg', '.jpeg']:
                # Update key name to use .webp extension
                s3_key_base = os.path.splitext(s3_key)[0]
                s3_key = f"{s3_key_base}.webp"
                content_type = 'image/webp'

                print(f"🖼️  Converting & Uploading: {relative_path} -> {s3_key} [{content_type}]", flush=True)

                try:
                    buffer = io.BytesIO()
                    with Image.open(local_file_path) as img:
                        # Ensure color mode is supported by WebP (RGB, RGBA, L, LA)
                        if img.mode not in ("RGB", "RGBA", "L", "LA"):
                            img = img.convert("RGBA" if "transparency" in img.info or img.mode == "P" else "RGB")
                        
                        # Save converted WebP to memory buffer
                        img.save(buffer, format="WEBP", quality=webp_quality)

                    buffer.seek(0)  # Reset buffer stream pointer
                    ...
                    print(f"❌ Failed to convert/upload {file}: {e}", flush=True)
                    fail_count += 1

            # --- Standard Upload Logic for non-convertible files ---
            else:
    ...
    print(f"\n==========================================")
    print(f"⚡ Sync Complete! Successfully processed/uploaded: {success_count} files.")
    if fail_count > 0:
        print(f"⚠️ Failures encountered: {fail_count} files.")
    print(f"==========================================\n")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Pure Python S3/MinIO Folder Sync Engine with WebP Conversion")
    ...
    parser.add_argument("--quality", help="WebP compression quality (1-100, default: 80)", type=int, default=80)
